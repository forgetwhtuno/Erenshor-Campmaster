...
{
    // Standalone/Hub UI readiness policy. Canonical v1 (integration handoff), readiness-only
    // subset - Campmaster has no dedicated panel or standalone launcher (see
    // SUITE_UI_MOD_CONTRACT.md's panel table), so only the IsGameplayReady()/RawGameplayReady()
    // gate is needed here, to serve as an outer safety check on control-API actions such as
    // relaxHere/relaxOff. No compile-time dependency on Suite Hub.
    // See CONTRACT_RECONCILIATION.md "Readiness contract".
